...
This "feature" we get for free highlights an important consideration when using auto-diff. We only differentiate the input variables with respect to the branch the particular run of the program took. We get correct derivatives, but conditional statements can make our function non-differentiable in some places. For example the `relu()` function used in neural networks can be written `max(0, x)`, which contains a conditional (`0 if x < 0 else x`), and its not really differentiable at `x==0` (we might say it's "subdifferentiable"). Furthermore, if we had any random numbers that are used in these conditionals, our derivatives are only valid for that particular sample from the random number generator. This is a subtle point, but important to keep in mind.
